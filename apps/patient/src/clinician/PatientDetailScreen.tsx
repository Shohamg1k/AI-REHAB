...
export function PatientDetailScreen({ patientId, onBack }: { patientId: string; onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-16">
      <button type="button" onClick={onBack} className="text-body-sm text-brand underline w-fit">
        ← Back to roster
      </button>

      <div>
        <h1 className="text-heading-20 text-text-primary">Session history</h1>
        {error && <p className="text-body-sm text-danger mt-4">{error}</p>}
        {sessions === null && !error && <p className="text-body-sm text-text-secondary mt-4">Loading…</p>}
        {sessions && sessions.length === 0 && (
          <p className="text-body-sm text-text-secondary mt-4">No sessions synced yet.</p>
        )}
        {sessions && sessions.length > 0 && (
      ...
      <div className="rounded-lg border border-border bg-surface p-16">
        <h2 className="text-title text-text-primary">Assign a program</h2>
        <div className="mt-12 flex flex-col gap-12">
            ...
            return (
              <div key={exercise.id} className="flex items-center gap-12">
                ...
                <input
                  ...
                  className="w-16 min-h-touch rounded-md border border-border px-8 text-center"
          ...
          <label className="flex flex-col gap-4">
            <span className="text-label text-text-secondary">Notes (optional)</span>
            <input
          ...
          {saved && <p className="text-body-sm text-success">Program assigned.</p>}
