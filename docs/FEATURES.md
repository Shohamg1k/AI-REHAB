...
|---|---|---|---|---|:--:|:--:|:--:|
| **A1** | Live pose tracking — full-body skeleton, single camera, no wearables | fast | M | — | | **MVP** | |
| **A2** | Per-rep form scoring — joint angles, ROM, tempo, symmetry vs reference ranges | fast | L | A1, A6 | | **MVP** | |
| **A3** | Live corrective coaching — visual + spoken cues mid-exercise, with cooldowns | fast | M | A2 | | **MVP** | |
| **A4** | Automatic rep and set counting | fast | M | A1 | | **MVP** | |
| **A5** | Compensation pattern detection — back instead of knee, favouring one side | fast | L | A2 | | FF | |
| **A6** | Exercise library with reference models (the exercise DSL) | — | M | — | | **MVP** | |
| **A7** | Camera setup coach + persistent capture-quality score | fast | M | A1 | ● | **MVP** | |
| **A8** | Personal ROM baseline — score against this patient, not a population average | fast | M | A2, G1 | ● | FF | |
| **A9** | Session replay with skeleton scrub (no video — rendered from stored landmarks) | plat | M | G1 | ● | FF | |
| **A10** | Bilateral comparison — injured vs uninjured as a headline metric | slow | S | A2, A8 | ● | FF | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **B1** | Movement-based pain estimation from pose deviations, guarding, asymmetry | slow | L | A2 | | FF | |
| **B2** | Self-report fusion — 1–5 scale + tap-to-point body map | slow | S | — | | **MVP** | |
| **B3** | Region-level pain mapping, per rep and per session | slow | M | B1, B2, G1 | | FF | |
| **B4** | Pain trend visualisation across sessions | slow | M | B3 | | FF | |
| **B5** | Pain-linked rep bookmarking — one button: *that one hurt* | fast | S | A4, G1 | ● | **MVP** | |
| **B6** | Inference as question generator, never a standalone verdict | slow | S | B1 | ● | FF | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **C1** | Session narration and encouragement | slow | M | G1 | | FF | |
| **C2** | Rest-period check-ins — "how did that set feel?" | slow | S | C1 | | FF | |
| **C3** | Voice Q&A — plain-language, non-diagnostic answers | slow | M | C1, E4 | | Later | |
| **C4** | Adjustable coaching persona — encouraging / clinical / concise | slow | S | C1 | | Later | |
| **C5** | Difficulty-aware tone shifts on frustration or fatigue | slow | M | C1, A2 | | Later | |
| **C6** | "Why this exercise" rationale cards | — | S | A6 | ● | **MVP** | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **D1** | Session-to-session adaptation of intensity, selection, pacing | slow | L | G1, B3, E5 | | FF | |
| **D2** | Progression policy — deterministic rule table now, learned policy later | slow | M | D1 | | FF | |
| **D3** | Plateau and regression detection | slow | M | D1, G2 | | Later | |
| **D4** | Pre-session readiness check that adjusts today's intensity | slow | S | D1 | | FF | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **E1** | Real-time safety gating — frame-level block/downgrade | fast | M | A2 | | **MVP** | |
| **E2** | Fall / instability detection during standing exercises | fast | M | A1, E1 | | FF | |
| **E3** | Escalation guidance — contact your clinician / emergency services | — | S | E1 | | **MVP** | |
| **E4** | Non-diagnostic framing throughout, snapshot-tested | — | S | — | | **MVP** | |
| **E5** | Session-level supervisor — filters the allowed exercise set | slow | S | B3 | | FF | |
| **E6** | Per-patient clinician threshold overrides | slow | M | E1, F6 | ● | Later | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **F1** | Automated session and weekly reports | slow | L | G1, G2 | | FF | |
| **F2** | Exportable clinician packet (PDF) | slow | M | F1 | | FF | |
| **F3** | Multi-session trend view — progressing / plateaued / regressing | slow | M | F1, G2 | | Later | |
| **F4** | Plain-language explanations of movement and pain data | slow | M | F1 | | Later | |
| **F5** | Full audit trail, traceable end to end | plat | S | G1 | | FF | |
| **F6** | Program builder + one-click assign, with template library | plat | L | G3, A6 | ● | Later | |
| **F7** | Flag triage inbox — ranked daily worklist across the caseload | plat | L | F1, G3 | ● | Later | |
| **F8** | Rep-anchored clinician annotations | plat | M | A9, F9 | ● | Later | |
| **F9** | Clinician-to-patient messaging | plat | M | G3 | | Later | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **G1** | Structured append-only session event log | plat | M | — | | **MVP** | |
| **G2** | Shared state and projections over the event log | plat | M | G1 | | FF | |
| **G3** | Auth and roles — patient / clinician / caregiver | plat | M | — | | FF | |
| **G4** | Multi-tenancy and row-level security | plat | M | G3 | | FF | |
| **G5** | Consent and data-sharing control centre with patient-visible access log | plat | M | G3 | ● | FF | |
| **G6** | Offline / local-first with deferred sync | plat | L | G1 | | FF | |
| **G7** | Confidence gating — low-confidence sessions marked, not averaged in | plat | S | A7, G1 | ● | FF | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **H1** | Streaks and session history calendar | plat | S | G1 | | FF | |
| **H2** | Milestone and progress celebrations | plat | S | A8, G2 | | FF | |
| **H3** | Gentle re-engagement nudges after a missed session | plat | M | G3 | | Later | |
| **H4** | Post-session summary card | — | S | G1 | | **MVP** | |
| **H5** | Caregiver / family visibility (opt-in) | plat | M | G3, G5 | | Later | |
| **H6** | Drop-out risk scoring — nudge before they quit, not after | slow | M | G2, H3 | ● | Later | |
| **H7** | Guest session without an account | plat | S | G1 | ● | FF | |
| **H8** | Gamified variety packs — reach-based mini-games | fast | L | A1, A6 | | Later | |
| **H9** | Multi-language, large text, high contrast, cue captions | — | L | — | | Later | |
| **H10** | Wearable integration for heart rate / recovery context | plat | L | G3 | | Later | |
...
| ID | Feature | Loop | Effort | Depends on | New | Rec | Pick |
|---|---|---|---|---|:--:|:--:|:--:|
| **I1** | Exercise authoring studio — record, auto-derive ranges, tune, publish | — | L | A2, A6 | ● | Later | |
| **I2** | Evaluation harness + fixture replay in CI | — | M | A2 | ● | **MVP** | |
| **I3** | Mobile shell (React Native) reusing the TypeScript core | — | L | A1–A4 | ● | Later | |
