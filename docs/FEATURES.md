...
| **H9** | Multi-language, large text, high contrast, cue captions | — | L | — | Multi-language **partial**: English + Hindi across spoken cues, captions and voice settings, plus a bundled Hindi neural voice (ADR-0011). **Large text partly done**: responsive column + viewport-scaled in-session overlay, readable at distance, phone/iPad/desktop. High contrast not started. | Later | OUT |
